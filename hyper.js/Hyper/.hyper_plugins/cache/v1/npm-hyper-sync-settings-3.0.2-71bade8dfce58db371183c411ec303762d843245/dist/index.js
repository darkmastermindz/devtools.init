...
const DIR_HOME = () => {
  ...
  return process.platform === 'win32' ? electron.app.getPath('userData') : os.homedir();
};
...
var notify = (({
  ...
  if (!electron.app || !electron.app.isReady()) return;
...
var checkForMissingSettings = (async () => {
  ...
  if (!hyperApp$1.config || typeof hyperApp$1.config.getConfig !== 'function') {
  ...
  const hyperConfig = hyperApp$1.config.getConfig().syncSettings || {
    quiet: false
  };
